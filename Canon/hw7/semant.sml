...
fun transExp (venv, tenv, exp, level: Tr.level, break) =
  let fun
	...
	in
		...
		SOME(Env.FunEntry({level=declevel, label, formals, result})) => (checkArgs(formals, args, pos);
      {exp=(Tr.callexpIR(declevel, level, label, argExpList)), ty=result})
	     |  SOME(_) => (error pos ("symbol not function " ^ S.name func); {exp=(Tr.nilIR()), ty=E.ERROR})
	     |  NONE => (error pos ("Error: undefined function " ^ S.name func); {exp=(Tr.nilIR()), ty=E.ERROR})
	end
		  ...
		  (
			...
			(
			  ...
			  in
				  ...
				  {exp=Tr.arrayIR(#exp (trexp size), #exp (trexp init)), ty=E.ARRAY(ty, unique)})
			      else
				  (error pos  "Error: integer expression expected in array initialization"; {exp=(Tr.nilIR()), ty=E.ERROR})
			  end
			)
	  ...
	  (case trvar v of
         ...
         fun getIndex ([], id, cur) = cur
          | getIndex (h :: t, id, cur) = if h = id then cur else getIndex(t, id, cur + 1)
	       in
		   {exp=Tr.fieldIR(#exp (trvar v), getIndex(map #1 fields, id, 0)), ty=getFieldType(recGen, id, pos)}
	       end
	  ...
	  in
	      (case trvar v of
		   {exp=_, ty=E.ARRAY(arrTy, unique)} => (checkIfInteger(trexp subExp, pos);
							   {exp=Tr.subscriptIR(#exp (trvar v), #exp (trexp subExp)), ty=actualTy arrTy})
...
fun transProg exp =
  let
      ...
      val {exp=mainExp, ty=mainTy} = (transExp (Env.base_venv, Env.base_tenv, exp, mainlevel, mainlabel))
      (*val _ = findescapes mainExp*)
    in
