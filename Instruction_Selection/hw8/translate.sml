functor TranslateFun(structure F: FRAME
                     ...
                     sharing type F.label = T.label = IR.label= Canon.label
                         and type T.temp = IR.temp
                         and type F.exp = IR.exp
                         and type F.stm = IR.stm= Canon.stm) :> TRANSLATE
...
struct
  ...
  fun procEntryExit({level=level', body=body'}) =
    let
      ...
      val b = (Canon.Canonize procedureBody (T.newlabel()))
	      val y = (F.procfrag levelFrame b)
    in
